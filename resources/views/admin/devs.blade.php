                ...
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    <div class="card">
                        <div class="header">
                            <h2>
                                ВСЕ АВТОРЫ
                                <span class = "badge bg-red">{{$devs->count()}}</span>
                            </h2>
                                ...
                                <table class="table table-bordered table-striped table-hover dataTable js-exportable">
                                   ...
                                   @foreach($devs as $key=>$dev)
                                    <tr>
                                            <td>{{$key +1}}</td>
                                            <td>{{$dev->name}}</td>
                            <td><span class = "badge bg-info">{{$dev->posts_count }}</span></td>
                                            <td>{{$dev->comments_count }}</td>
                                            <td>{{$dev->created_at->toDateString()}}</td>
                                            <td class = "text-center">
                                            


                        <button class = "btn btn-danger wave-effects" type = "button" onClick = "deleteDev({{$dev->id}})">  
                        <i class = "material-icons">delete</i>
                        </button>

                        <form id = "delete-form-{{$dev ->id}}"action = "{{route('admin.dev.destroy',$dev->id)}}" method = "POST"
                        style = "display:none;">
                            @csrf
                            @method('DELETE')
                            </form>              
